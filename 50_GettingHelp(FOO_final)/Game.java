...
public class Game {
    ...
    public void maelstromEncounter() {
        ...
        if (roomExists(currentX + 1 , currentY - 2) && getRoomType(currentX + 1, currentY - 2) instanceof EmptyRoom) {
    ...
    private void moveMaelstromToEmptyRoom(int currentX, int currentY) {
        do {
            int x = rand.nextInt(size - 1);
            int y = rand.nextInt(size - 1);
            if (getRoomType(x, y) instanceof EmptyRoom) {
    ...
    public Room getRoomType(int x, int y) {
        if (!roomExists(x, y)) {
            return null;
        }
